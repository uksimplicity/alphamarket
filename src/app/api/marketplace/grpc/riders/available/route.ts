...
export async function GET(req: Request) {
  ...
  const urls = Array.from(
    new Set([
      `${API_V1_BASE}/marketplace/grpc/riders/available${search}`,
      `${API_BASE}/marketplace/grpc/riders/available${search}`,
    ])
  );
  ...
  if (!res) {
    return new Response(
      JSON.stringify({
        error: "Marketplace rider availability endpoint not found on upstream.",
        tried: urls,
      }),
