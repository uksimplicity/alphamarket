...
export async function POST(req: Request) {
  ...
  const targets = Array.from(
    new Set([`${API_V1_BASE}/user/change-password`, `${API_BASE}/user/change-password`])
  );
  ...
  if (!res) {
    return new Response(
      JSON.stringify({ error: "Change password endpoint not found on upstream.", tried: targets }),
