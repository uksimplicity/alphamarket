...
export async function GET(
  ...
  const urls = Array.from(
    new Set([
      `${API_V1_BASE}/user/orders/${safeId}${search}`,
      `${API_BASE}/user/orders/${safeId}${search}`,
    ])
  );
  ...
  if (!res) {
    return new Response(
      JSON.stringify({ error: "User order details endpoint not found on upstream.", tried: urls }),
