...
export async function GET(
  ...
  const urls = Array.from(
    new Set([
      `${API_V1_BASE}/seller/categories/${safeId}${search}`,
      `${API_BASE}/seller/categories/${safeId}${search}`,
    ])
  );
  ...
  return new Response(JSON.stringify({ error: "Seller category details endpoint not found.", tried: urls }), {
    status: 502,
