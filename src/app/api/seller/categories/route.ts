...
export async function GET(req: Request) {
  ...
  const urls = Array.from(
    new Set([
      `${API_V1_BASE}/seller/categories${search}`,
      `${API_BASE}/seller/categories${search}`,
    ])
  );
  ...
  return new Response(JSON.stringify({ error: "Seller categories endpoint not found.", tried: urls }), {
    status: 502,
