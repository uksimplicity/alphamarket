...
export async function GET(
  ...
  const urls = Array.from(
    new Set([
      `${API_V1_BASE}/seller/products/${safeId}/attributes${search}`,
      `${API_BASE}/seller/products/${safeId}/attributes${search}`,
    ])
  );
  ...
  return new Response(JSON.stringify({ error: "Seller product attributes endpoint not found.", tried: urls }), {
    status: 502,
