...
export async function GET(req: Request) {
  ...
  const urls = Array.from(
    new Set([`${API_V1_BASE}/seller/brands${search}`, `${API_BASE}/seller/brands${search}`])
  );
  ...
  return new Response(JSON.stringify({ error: "Seller brands endpoint not found.", tried: urls }), {
    status: 502,
