...
export async function GET(
  ...
  const urls = Array.from(
    new Set([`${API_V1_BASE}/seller/brands/${safeId}${search}`, `${API_BASE}/seller/brands/${safeId}${search}`])
  );
  ...
  return new Response(JSON.stringify({ error: "Seller brand details endpoint not found.", tried: urls }), {
    status: 502,
