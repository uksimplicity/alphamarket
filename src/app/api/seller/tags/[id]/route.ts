...
export async function GET(
  ...
  const urls = Array.from(
    new Set([`${API_V1_BASE}/seller/tags/${safeId}${search}`, `${API_BASE}/seller/tags/${safeId}${search}`])
  );
  ...
  return new Response(JSON.stringify({ error: "Seller tag details endpoint not found.", tried: urls }), {
    status: 502,
