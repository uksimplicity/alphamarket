...
export async function GET(req: Request) {
  ...
  const urls = Array.from(
    new Set([`${API_V1_BASE}/seller/attributes${search}`, `${API_BASE}/seller/attributes${search}`])
  );
  ...
  return new Response(JSON.stringify({ error: "Seller attributes endpoint not found.", tried: urls }), {
    status: 502,
