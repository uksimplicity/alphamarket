...
export async function GET(
  ...
  const urls = Array.from(
    new Set([
      `${API_V1_BASE}/seller/attributes/${safeId}${search}`,
      `${API_BASE}/seller/attributes/${safeId}${search}`,
    ])
  );
  ...
  return new Response(JSON.stringify({ error: "Seller attribute details endpoint not found.", tried: urls }), {
    status: 502,
