...
export async function POST(
  ...
  const urls = Array.from(
    new Set([
      `${API_V1_BASE}/buyer/orders/${safeId}/confirm-delivery`,
      `${API_BASE}/buyer/orders/${safeId}/confirm-delivery`,
    ])
  );
  ...
  if (!res) {
    return new Response(
      JSON.stringify({
        error: "Buyer confirm-delivery endpoint not found on upstream.",
        tried: urls,
      }),
