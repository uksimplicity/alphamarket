...
export async function POST(req: Request) {
  ...
  const urls = Array.from(new Set([`${API_V1_BASE}/buyer/orders`, `${API_BASE}/buyer/orders`]));
  ...
  if (!res) {
    return new Response(
      JSON.stringify({ error: "Buyer orders endpoint not found on upstream.", tried: urls }),
