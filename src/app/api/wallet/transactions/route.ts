...
export async function GET(req: Request) {
  ...
  const urls = Array.from(
    new Set([
      `${API_V1_BASE}/wallet/transactions${search}`,
      `${API_BASE}/wallet/transactions${search}`,
    ])
  );
  ...
  if (!res) {
    return new Response(
      JSON.stringify({ error: "Wallet transactions endpoint not found on upstream.", tried: urls }),
