...
export async function GET(req: Request) {
  ...
  const urls = Array.from(new Set([`${API_V1_BASE}/wallet/stats`, `${API_BASE}/wallet/stats`]));
  ...
  if (!res) {
    return new Response(
      JSON.stringify({ error: "Wallet stats endpoint not found on upstream.", tried: urls }),
